...
export async function getRouteElevationProfile(routeGeoJSON, navGlMap) {
    ...
    const baseStepKm = 0.2;
    const stepKm = Math.max(baseStepKm, totalDistKm / maxSamples);
    const distances = [];
    const elevations = [];
    const slopes = [];
    ...
    while (d <= totalDistKm + 1e-6) {
        ...
        let elev = 0;
        try {
            const val = navGlMap.queryTerrainElevation({ lng, lat });
            elev = typeof val === 'number' && isFinite(val) ? val : 0;
        } catch {
            elev = 0;
        }
